...
def get_all_data_loaders(conf):
    ...
    train_loader, train_count = get_data_loader_folder(
        conf['data_root'], 'train', conf['n_datasets'], conf['label_use'], batch_size, True, resize_to, num_workers)
    test_loader, test_count = get_data_loader_folder(
        conf['data_root'], 'test', conf['n_datasets'], conf['label_use'], batch_size, False, resize_to, num_workers)

    return train_loader, test_loader, train_count, test_count


def get_data_loader_folder(data_root, mode, n_datasets, label_use, batch_size, shuffle, resize_to=(284, 284), num_workers=4):

    dataset = ImageFolder(data_root, n_datasets, label_use, mode, resize_to)
    loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=shuffle,
                        drop_last=True, num_workers=num_workers, pin_memory=True)

    total_class_count = np.array([0, 0, 0, 0])

    for counts in dataset.classes_count:
        total_class_count += counts

    return loader, total_class_count
