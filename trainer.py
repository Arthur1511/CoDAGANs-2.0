...
class MUNIT_Trainer(nn.Module):
    ...
    def __init__(self, hyperparameters, devices, loss_weights):
        ...
        self.sup = None
#         if hyperparameters['arch'] == 'unet':
        #self.sup = UNet(hyperparameters['input_dim'], hyperparameters['n_classes'], hyperparameters['sup']).cuda(self.cuda1)
        self.sup = resnet50(num_classes=4).cuda(self.cuda1)
    ...
    def sup_criterion(self, pred, target):
        ...
        return F.cross_entropy(pred, target, weight=self.loss_weights, ignore_index=-1, reduction='mean')
    ...
    def sample(self, x_a, x_b, y_a, y_b, ind_a, ind_b):
        ...
        msk_a, msk_b = [], []
        pred_a, pred_b = [], []
        print((x_a.unsqueeze(0)).shape)
        for i in range(x_a.size(0)):

            print((x_a[i].unsqueeze(0)).shape)
            print((self.one_hot_img[ind_a, 0]).shape)
            hot_x_a = torch.cat(
            ...
            prob_a = functional.softmax(p_a)
            _, preds_a = torch.max(prob_a, 1)

            prob_b = functional.softmax(p_b)
            _, preds_b = torch.max(prob_b, 1)

            pred_a.append(preds_a.cpu())
            pred_b.append(preds_b.cpu())

        '''
            hot_c_ab = torch.cat([c_a, self.one_hot_c[ind_b, 0].unsqueeze(0)], 1)
            hot_c_ba = torch.cat([c_b, self.one_hot_c[ind_a, 0].unsqueeze(0)], 1)
    ...
    def sup_forward(self, x, d_index, hyperparameters):

        self.sup.eval()

        #s_a = self.s_a
        #content, s_a_fake = self.gen.encode(x)

        # Encoding content image.

        #one_hot_x = torch.cat([x, self.one_hot_img[d_index, 0].unsqueeze(0)], 1)
        one_hot_x = torch.cat([x, self.one_hot_img[d_index]], 1)
