...
class VAEGen(nn.Module):
    ...
    def __init__(self, input_dim, params):
        ...
        self.dec = Decoder(n_downsample, n_res, self.enc.output_dim + params['n_datasets'], input_dim, res_norm='in', activ=activ, pad_type=pad_type)
...
class UNet(nn.Module):
    ...
    def __init__(self, input_channels, num_classes, params):
        ...
        self.enc4 = _EncoderBlock(params['dim'] * 4, params['dim'] * 8, dropout=True)
        
        
        # Decoders.
        self.center = _DecoderBlock(params['dim'] * 8, params['dim'] * 16, params['dim'] * 8)
        
        self.dec4 = _DecoderBlock(params['dim'] * 16, params['dim'] * 8, params['dim'] * 4)
        self.dec3 = _DecoderBlock(params['dim'] * 8, params['dim'] * 4, params['dim'] * 2)
        self.dec2 = _DecoderBlock(params['dim'] * 2, params['dim'] * 1, params['dim'] * 1)
        
        self.dec1 = nn.Sequential(
            nn.Dropout2d(),
            nn.Conv2d(params['dim'] * 1, params['dim'] * 1, kernel_size=3, padding=1),
    ...
    def forward(self, x, use=None, only_prediction=True):
        ...
        dec4 = self.dec4(torch.cat([center, F.interpolate(enc4, center.size()[2:], mode='bilinear')], 1)) # 32 -> 64.
        dec3 = self.dec3(torch.cat([dec4, F.interpolate(enc3, dec4.size()[2:], mode='bilinear')], 1)) # 64 -> 128.
...
def resnet50(pretrained = False, progress = True, num_classes=4):

    return _resnet('resnet50', Bottleneck, [2, 2, 2, 2], pretrained, progress, num_classes)
